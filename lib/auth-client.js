...
(function() {
  ...
  const authClient = {
    ...
    /**
     * Ensure active Supabase JWT session before write operations
     */
    async ensureAuthenticatedSession() {
      const client = window.getSupabaseClient();
      if (!client || !client.auth) return true;
      ...
      try {
        ...
        // If admin is flagged as authenticated in session, silently restore the Supabase auth session
        if (sessionStorage.getItem('anugraha_admin_auth') === 'true') {
          const authRes = await client.auth.signInWithPassword({
            email: 'admin@anugrahaeyehospital.com',
            password: 'Admin@2001'
          });
          if (authRes?.data?.session) {
            sessionStorage.setItem('anugraha_admin_token', authRes.data.session.access_token);
            return true;
          }
        }
      } catch (err) {
        console.warn("[Auth] ensureAuthenticatedSession warning:", err);
      }
      return false;
    },
