  ...
  const cmsClient = {
    ...
    async uploadToCloudStorage(fileOrDataUrl, folder = 'general', customFilename = null) {
      ...
      if (typeof fileOrDataUrl === 'string' && fileOrDataUrl.startsWith('data:')) {
        ...
        filename = customFilename ? `${customFilename}.${ext}` : `image_${Date.now()}.${ext}`;
      } else if (fileOrDataUrl instanceof Blob || fileOrDataUrl instanceof File) {
        blob = fileOrDataUrl;
        mimeType = fileOrDataUrl.type || 'image/jpeg';
        filename = fileOrDataUrl.name || customFilename || `image_${Date.now()}.jpg`;
      } else {
      ...
      if (blob.size > maxMB * 1024 * 1024) {
        throw new Error(`Image size exceeds the ${maxMB}MB limit. Please choose a smaller image.`);
      }
      ...
      console.log(`[Storage] Uploaded image successfully: ${result.url}`);
      return result;
    },
