...
(function() {
  ...
  const cmsClient = {
      ...
      try {
        ...
        if (adminRes.data && adminRes.data.length > 0) {
          result.administration = adminRes.data.map(a => ({
            ...
            position: a.role,
            degrees: a.degrees || '',
            qualifications: a.degrees || '',
            department: a.department || '',
            bio: a.bio || '',
            desc: a.bio || '',
            photo: a.photo || '',
            tenure: a.tenure || (window.DEFAULT_DATA?.administration?.find(d => d.id === a.id)?.tenure || '10 Years'),
    ...
    async saveSetting(settingKey, settingValue) {
      ...
      try {
        if (window.authClient && typeof window.authClient.ensureAuthenticatedSession === 'function') {
          await window.authClient.ensureAuthenticatedSession();
        }
        ...
        return { success: true, key: settingKey, data: settingValue };
      } catch (e) {
    ...
    async saveAdministration(member) {
      ...
      try {
        ...
        const payload = {
          ...
          degrees: member.qualifications || member.degrees || '',
          department: member.department || '',
          bio: member.desc || member.bio || '',
          photo: member.photo || '',
    ...
    /**
     * Persist all administration staff members in bulk
     */
    async saveAllAdministration(teamList) {
      const client = window.getSupabaseClient();
      if (!client || !Array.isArray(teamList)) return { success: false, error: "Invalid data" };
      ...
      try {
        ...
        const rows = teamList.map((m, idx) => ({
          id: m.id,
          name: m.name,
          role: m.role || m.position || '',
          degrees: m.qualifications || m.degrees || '',
        ...
        if (error) throw error;
        this.logAuditEvent('UPSERT_ALL_ADMIN_MEMBERS', `${rows.length} members updated`);
        return { success: true, data };
      } catch (e) {
        console.error("[CMSClient] Failed to bulk save admin members:", e);
        return { success: false, error: e.message || e };
      }
    },
    ...
    async deleteDoctor(id) {
      ...
      try {
        ...
        const { error } = await client.from('cms_doctors').delete().eq('id', id);
        if (error) throw error;
        return true;
      } catch (e) {
        console.warn("[CMSClient] Delete doctor error:", e);
        return false;
      }
    },
    ...
    async deleteFacility(id) {
      ...
      try {
        ...
        const { error } = await client.from('cms_facilities').delete().eq('id', id);
        if (error) throw error;
        return true;
      } catch (e) {
        console.warn("[CMSClient] Delete facility error:", e);
        return false;
      }
    },
    ...
    async deleteAdminMember(id) {
      ...
      try {
        ...
        const { error } = await client.from('cms_administration').delete().eq('id', id);
        if (error) throw error;
        return true;
      } catch (e) {
        console.warn("[CMSClient] Delete admin member error:", e);
        return false;
      }
    },
    ...
    async deleteService(id) {
      ...
      try {
        ...
        console.warn("[CMSClient] Delete service error:", e);
        return false;
      }
    },
    ...
    async deleteEquipment(id) {
      ...
      try {
        ...
        console.warn("[CMSClient] Delete equipment error:", e);
        return false;
      }
    },
